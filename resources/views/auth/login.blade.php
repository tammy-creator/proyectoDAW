@extends('layouts.main', ['class' => 'off-canvas-sidebar', 'activePage' => 'login', 'title' => __('Centro Proyecta')])
      ...
      <form class="form" method="POST" action="{{ route('login') }}">
        ...
        <div class="card card-login card-hidden mb-3">
          <div class="card-header card-header-primary text-center">
            <h3 class="card-title"><strong>{{ __('Acceso a tu cuenta Proyecta') }}</strong></h3>
          </div>
          <div class="card-body">
            <p class="card-description text-center">{{ __('Introduce tus credenciales') }} </p>
      ...
      <div class="row">
        ...
        <div class="col-6 text-right">
            <a href="{{ route('register') }}" class="text-light">
                <small>{{ __('Todavía no tienes cuenta?') }}</small>
            </a>
        </div>
      </div>
    </div>
